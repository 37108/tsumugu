import { PackageGraph } from "../../.components/diagram.jsx";
...
[`docs/designs/architecture/index.mdx`](./): the scanner, document model,
...
<PackageGraph
  id="graph"
  title="Workspace dependency direction"
  description="tsumugu depends on tsumugu-preset, and, only under the trust declaration, on tsumugu-renderer-mdx. The preset depends on renderer-markdown, renderer-html, theme-default and transformer-highlight. renderer-mdx depends on renderer-markdown and renderer-html. All four of those depend on tsumugu-core, which depends on none of them."
  layers={[
    [{ name: "tsumugu" }],
    [
      { name: "tsumugu-preset" },
      { name: "renderer-mdx", optional: true, note: "only under --trust" },
    ],
    [
      { name: "renderer-markdown" },
      { name: "renderer-html" },
      { name: "theme-default" },
      { name: "transformer-highlight" },
    ],
    [{ name: "tsumugu-core" }],
  ]}
  edges={[
    { from: "tsumugu", to: "tsumugu-preset" },
    { from: "tsumugu", to: "renderer-mdx", conditional: true },
    { from: "tsumugu-preset", to: "renderer-markdown" },
    { from: "tsumugu-preset", to: "renderer-html" },
    { from: "tsumugu-preset", to: "theme-default" },
    { from: "tsumugu-preset", to: "transformer-highlight" },
    { from: "renderer-mdx", to: "renderer-markdown" },
    { from: "renderer-mdx", to: "renderer-html" },
    { from: "renderer-markdown", to: "tsumugu-core" },
    { from: "renderer-html", to: "tsumugu-core" },
    { from: "theme-default", to: "tsumugu-core" },
    { from: "transformer-highlight", to: "tsumugu-core" },
  ]}
/>

Every arrow points downward, which is the rule rather than a drawing
convention: a package that had to be drawn pointing up would be a package that
broke it. The dashed box and the dashed arrow are the one edge that is made at
runtime rather than always.
...
[`docs/designs/architecture/index.mdx`](./) and
...
Every workspace under `packages/` is published to npm, on one version for all of
them; every workspace under `internal/` is `"private": true` and never is. The
current release is `0.8.0`, and `docs/designs/releasing.md` describes how one is
made.

Each published package carries full publication metadata: `exports`, `files`,
`license`, `repository`, `engines`, and `"type": "module"`. That was true before
anything was published, which is what kept the public surface explicit and
testable rather than assembled at release time.
...
| Vitest     | `^4.1.10`  | `vitest.config.ts`; see `docs/designs/testing.md` |
